...
def process_single_file(file_path: str, prompt: str) -> Dict:
    ...
    try:
        ...
        llm_response, llm_seconds = make_api_request(extracted_url, prompt)

        file_result["llm_response"] = llm_response
        file_result["llm_processing_time_seconds"] = llm_seconds
...
def process_files_batch(directory: str, num_files: int, prompt: str, max_workers: int = 5) -> Dict:
    ...
    if llm_times:
        results["summary"]["total_llm_processing_time_seconds"] = round(sum(llm_times), 3)
        results["summary"]["avg_llm_processing_time_seconds"] = round(
            sum(llm_times) / len(llm_times), 3
        )
